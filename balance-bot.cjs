...
const supabase = createClient(
  'https://rxysrxncfbiflsaguuaj.supabase.co',
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJ4eXNyeG5jZmJpZmxzYWd1dWFqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODEzODQyMzQsImV4cCI6MjA5Njk2MDIzNH0.-vYQ9bBmhqwCvsFEptluqvpFjhQ8yYEnf-RUnYsyn4w'
);
...
client.once('clientReady', () => {
  ...
  // Pazar günleri saat 23:00'da çalışacak cron job
  cron.schedule('0 23 * * 0', async () => {
    try {
      console.log('Siphone negative balances cron job tetiklendi.');
      const channel = await client.channels.fetch(ALLOWED_BALANCE_CHANNEL_ID);
      if (!channel) return;
      ...
      const negativePlayers = data?.data || [];
      ...
      console.error('Cron job hatasi:', err);
    }
  });
});
